                                      ...
                                      Packet *packet)
{
    ...
    if (strcmp(packet->getName(), "Transaction") == 0) {
    ...
    else if (strcmp(packet->getName(), "Block") == 0) {
...
void BlockchainApp::sendBlock(const Block& block)
{
    auto packet = new Packet("Block");

    const auto& blockChunk =
            makeShared<BlockChunk>();

    blockChunk->setBlockId(
            block.blockId);

    blockChunk->setPreviousBlockHash(
            block.previousBlockHash);

    blockChunk->setBlockHash(
            block.blockHash);

    blockChunk->setChunkLength(B(32));

    packet->insertAtBack(blockChunk);

    EV_INFO << "Broadcasting Block "
            << block.blockId
            << endl;
